	...
	public String discussionRules(String input) {
		...
		if (m1.matches() && !m2.matches()) {
			...
			if (m2_1.matches()) {
				...
				if (happyList.contains(keyword.toLowerCase())) {
					PrintMessage.messageFromBot("I'm glad to hear that you're " + keyword
							+ "! Keep it up by getting into healthy habits. Do you excercise?");
					String temp = PrintMessage.messageFromUser();
			...
			PrintMessage.messageFromBot("I'm sorry to hear that, make sure to block out time for yourself to unwind. \nRemeber, things you can't change aren't worth worrying about! Whats been stressing you out?");
			String temp = PrintMessage.messageFromUser().toLowerCase();
			...
			if(m3_1.find()) {
				...
				PrintMessage.messageFromBot("Sorry to hear that your work has been so stressful. Do you enjoy what you do?");
				temp = PrintMessage.messageFromUser().toLowerCase();
			...
			PrintMessage.messageFromBot("Do you excercise? It can help with sleeping better.");
			String temp = PrintMessage.messageFromUser().toLowerCase();
			...
			if (m7_1.find() || m7_2.find()) {
				PrintMessage.messageFromBot("Good for you! Excercise has been proven to improve concentration, helps you sleep better,"
						+ " and reduce anxiety and depression. Since you're still tired do you have a regular sleep schedule?");
				temp = PrintMessage.messageFromUser().toLowerCase();
				m7_1 = p7_1.matcher(temp);

				if(m7_1.find()) {
					PrintMessage.messageFromBot("Thats good, keeping a regular sleep schedule has been proven to improve sleep quality. Make sure you keep it up! Are you going to bed early enough?"
				...
				PrintMessage.messageFromBot("You should try it. Excercise has been proven to improve concentration, helps you sleep better,"
...
//					scanner1.nextLine();
			...
			PrintMessage.messageFromBot(sampleMessages[2]);
			String temp = PrintMessage.messageFromUser().toLowerCase();
			...
			if(m8_1.find() || m8_2.find()) {
				PrintMessage.messageFromBot("It's good that you're trying to improve yourself! Procrastination can also be a symptom of anxiety or depression. Do you feel anxious or depressed?");
//				scanner1.nextLine();
				temp = PrintMessage.messageFromUser().toLowerCase();
			...
			PrintMessage.messageFromBot("No case match");
			output = noMessages[(int) (Math.random() * noMessages.length)];
		}
